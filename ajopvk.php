<?php session_start();
    ...
    $password = "";
    $datatable = "ajokirja";
    $connection = new mysqli($server, $user, $password, $datatable);
    ...
    if(isset($_POST["ok"])){
        ...
        $driver=mysqli_real_escape_string($connection,$_POST['kuka']);
        $car=mysqli_real_escape_string($connection,$_POST['auto']);
        $kms=mysqli_real_escape_string($connection,$_POST['kilom']);
        $addresslist="";
        $street=$_POST["katu"];
        for($i=0; $i<count($street); $i++){
            $addresslist=$addresslist."$street[$i];";
        }
        if(!preg_match('/^[a-zA-Z0-9\s]+$/', $driver)){
            echo("Tarkista nimesi!");
        }else if(!isset($car)){
            echo("Lisää auton rekisterinumero");
        }else {
            $insert="Insert into ajotaulu(ajaja, auto, pvm, kilometrit, osoitteet) values('$driver', '$car', '$pvm' ,$kms, '$addresslist');" ;
            $added=$connection->query($insert);
        }
    }
?>
<!DOCTYPE>
...
<body>
    <nav class="navbar navbar-dark bg-dark">
        ...
        <ul class="nav nav-tabs">
            ...
            <a class="nav-link" href="/drivingapp/ajohistoria.php" style="color:white">Ajohistoria</a>
        </ul>
    </nav>
    ...
    <div class="container-fluid">
        <div id="lomakevastaus"> 
            ...
            } else if(isset($added) && $added==true){
